...
<script>
 ...
 var curVid='', started=false, rtt=[], barTimer=null, lastTarget=0, videoProxyTried=false, videoLoadTimer=null;
 ...
 // 직접 재생이 실패하거나(에러) 몇 초 안에 시작 못하면(에러 없이 멈춤 — 차 핫스팟이라 태블릿 인터넷 없음) 헤드유닛 중계(프록시)로 자동 폴백.
 function toProxy(){
   if(videoProxyTried || !curVid) return;
   videoProxyTried=true; if(videoLoadTimer){clearTimeout(videoLoadTimer);videoLoadTimer=null;}
   video.src='/vid?v='+encodeURIComponent(curVid); video.muted=true; video.load(); if(started) video.play().catch(function(){});
 }
 video.addEventListener('error',toProxy);
 // 직접재생이 데이터 로드에 성공하면 타임아웃 폴백 취소(인터넷 있는 환경).
 video.addEventListener('loadeddata',function(){ if(videoLoadTimer){clearTimeout(videoLoadTimer);videoLoadTimer=null;} });
 ...
 async function tick(){
   ...
   if(d.videoId && d.streamUrl && curVid!==d.videoId){
     ...
     // 인터넷 없는 태블릿은 직접재생이 에러 없이 멈추기만 함 → 4초 내 데이터 못 받으면 프록시로 전환.
     (function(vid0){ videoLoadTimer=setTimeout(function(){ if(curVid===vid0 && !videoProxyTried && video.readyState<3) toProxy(); },4000); })(curVid);
   }
